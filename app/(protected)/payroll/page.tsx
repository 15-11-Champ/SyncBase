...
export default function PayrollManagement() {
  ...
  const [payrollToDelete, setPayrollToDelete] = useState<StaffPayroll | null>(null)
  ...
  const handleDeletePayroll = async () => {
    if (!payrollToDelete || !currentTenant) return
  ...
  const handleSavePayroll = async () => {
    ...
    if (error) {
      console.error('Error inserting payroll:', error.message || error)
  ...
  // -------------------------
  if (isTenantLoading) {
    return <div className="flex-1 p-6 flex items-center justify-center">Loading...</div>
  }

  if (!currentTenant) {
    return <div className="flex-1 p-6 flex items-center justify-center">No tenant selected</div>
  }

  return (
    <div className="flex-1 p-6 overflow-auto">
      <div className="space-y-6">
        {showAlerts && missingAttendance.length > 0 && (
          <Alert className="bg-orange-50 border-orange-500 dark:bg-orange-950/20">
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                ...
                <p className="font-semibold">
                  {payrollToDelete?.staff_name}
                  {payrollToDelete?.staff_role ? ` — ${payrollToDelete.staff_role}` : ''}
                </p>
